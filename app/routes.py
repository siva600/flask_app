...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        # adding user, committing the changes so to add out user to database.
        flash(f'Account has been created for {form.username.data}! Please log in!', 'success')
...
def dashboard():
    ...
    page = request.args.get('page', 1, type=int)
    user_posts = Post.query.paginate(page=page, per_page=3)
